...
export default function CheckoutPage() {
    ...
    const handleSubmission = async (e: React.FormEvent) => {
        ...
        const customerPhoneText = sameAsWhatsapp
            ? formData.phone
            : `WhatsApp: ${formData.phone} | Call: ${callPhone}`

        const combinedAddress = `Email: ${formData.email}\nAddress: ${formData.address}`
        ...
        if (error) {
            ...
            // Send Email Notification using Resend via our API route
            try {
            ...
            <div className="max-w-2xl mx-auto">
                ...
                <div className="grid gap-8">
                    ...
                    <form onSubmit={handleSubmission} className="space-y-6 bg-white border-2 border-black/5 p-8 rounded-3xl shadow-sm">
                        ...
                        <div>
                            <label className="block text-sm font-bold text-muted-foreground uppercase tracking-wide mb-2">Email Address</label>
                            <input
                                type="email"
                                required
                                className="mt-2 block w-full rounded-xl border-2 border-black/10 bg-white text-foreground px-4 py-4 focus:border-primary focus:outline-none focus:ring-4 focus:ring-primary/20 sm:text-base font-bold placeholder-muted-foreground transition-all"
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2 mt-3 pl-1">
                                <input
                                    ...
                                    className="w-4 h-4 rounded text-primary focus:ring-primary border-black/10 cursor-pointer"
                                />
                                <label htmlFor="sameAsWhatsapp" className="text-sm font-semibold text-muted-foreground cursor-pointer select-none">
                                    Same number for calls
                                </label>
                            </div>
                        </div>
                        {!sameAsWhatsapp && (
                            <div className="animate-in fade-in slide-in-from-top-2 duration-200">
                                <label className="block text-sm font-bold text-muted-foreground uppercase tracking-wide mb-2">Phone Number (For Calls)</label>
                                <input
                                    type="tel"
